  ...
  <div class="createpost">
    <div class="container">
      <div class="main">
        ...
        <div>
          <button @click="openPreview">پیش نمایش تصویر</button>
        </div>
        <router-link to="/preview-blog">
          <button @click="uploadBlog">پیش نمایش پست</button>
        </router-link>
        <button @click="publishBlog">بارگذاری پست</button>
  ...
  methods: {
    ...
    openPreview() {
      ...
      const storageRef = firebase
        .storage()
        .ref(
          `documents/blogCoverPhotoPreview/${this.$store.state.blogPhotoName}`
        );
    ...
    publishBlog() {
      if (
        this.blogCataloge !== "انتخاب عنوان" &&
        this.blogTitle.length !== 0 &&
        this.blogHtml !== "متن پست"
      ) {
        if (this.file) {
          const storageRef = firebase.storage().ref();
          const docRef = storageRef.child(
            `documents/blogPostPreview/${this.$store.state.blogPhotoName}`
          );
        ...
        (this.error = true), (this.errorMsg = "لطفا فیلدهای خالی پر شوند");
        setTimeout(() => {
          this.error = false;
        }, 5000);
      }
...
button {
  ...
  box-shadow: 2px 2px 2px rgba(0, 0, 0, 0.665);
}
</style>
